...
::: {#qua-pop-caracteristicas-gerais-das-estimativas-populacionais}
...
Use estimativas populacionais quando o objetivo for padronizar uma contagem pelo tamanho da população sob risco ou da população-alvo (ver @qua-pop-perguntas-comuns-que-usam-populacao-como-denominador).

::: {#qua-pop-perguntas-comuns-que-usam-populacao-como-denominador}
...
Dados populacionais podem vir de observação direta ou de modelos. Censos demográficos buscam enumerar a população residente e levantar características demográficas, sociais, econômicas e domiciliares. Entre censos, estimativas e projeções atualizam o tamanho da população com métodos demográficos e estatísticos (ver @qua-pop-diferencas-entre-fontes-populacionais).

::: {#qua-pop-diferencas-entre-fontes-populacionais}
...
A escolha do denominador deve vir da pergunta analítica, não da disponibilidade imediata da tabela. A população total do município serve para alguns indicadores, mas é inadequada para muitos outros (ver @qua-pop-denominadores-frequentes-em-indicadores-de-saude).

::: {#qua-pop-denominadores-frequentes-em-indicadores-de-saude}
...
As fontes citadas usam métodos e finalidades diferentes. Discrepâncias são esperadas, especialmente em municípios pequenos, em áreas com migração intensa, em anos próximos ao censo, em territórios que sofreram alteração de limites e em grupos etários muito específicos (ver @qua-pop-razoes-comuns-para-discrepancias-entre-estimativas).

::: {#qua-pop-razoes-comuns-para-discrepancias-entre-estimativas}
...
Em séries históricas, preserve a fonte e a versão da população associadas ao resultado final. Isso evita que uma atualização posterior do denominador torne tabelas e gráficos irreprodutíveis.
...
- [Siaps — Sistema de Informação para a Atenção Primária à Saúde — seção “Qual denominador usar?”](sisaps.html#qual-denominador-usar). Na edição impressa, p. 288.

- [SI-PNI — Sistema de Informações do Programa Nacional de Imunizações — seção “Cobertura vacinal e denominadores”](sipni.html#cobertura-vacinal-e-denominadores). Na edição impressa, p. 315.

- [Inquéritos e pesquisas de saúde — seção “Pesos, desenho e variância”](inqueritos.html#pesos-desenho-e-variância). Na edição impressa, p. 408.
