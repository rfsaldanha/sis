...
Este é um dos sistemas de informação de saúde que  recebe novos registros com maior frequência, junto com o Sistema de Informações Ambulatoriais -- SIA (@sec-sia). A análise destes dados requerem estratégias específicas para lidar com grandes bases de dados, utilizando, em geral, soluções de banco de dados relacionais que permitam um processamento analítico on-line (OLAP) eficiente.
