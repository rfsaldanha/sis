...
A tabela funciona como uma triagem inicial. Se a pergunta é sobre estabelecimentos, `ST` costuma ser o ponto de partida; se a pergunta é sobre recursos específicos, os demais arquivos entram como camadas complementares. O erro mais frequente é começar por um arquivo detalhado, como `PF` ou `LT`, e depois tratar suas linhas como se fossem estabelecimentos únicos.
...
Os recortes institucionais da tabela não são intercambiáveis. Rede SUS, rede pública, rede privada e filantrópica podem se sobrepor dependendo do critério usado. Em uma análise de oferta, o recorte por leitos SUS pode ser mais adequado; em uma análise de gestão, natureza jurídica e esfera administrativa podem ser mais informativas.
...
- [Organização dos SIS — seção “Sistemas cadastrais, financeiros e de gestão”](organizacao.html#sistemas-cadastrais-financeiros-e-de-gestão). Na edição impressa, p. 35.

- [SIH — Sistema de Informações Hospitalares do SUS — seção “Exemplo: fluxos de internação”](sih.html#exemplo-fluxos-de-internação). Na edição impressa, p. 110.

- [SISAPS — Sistema de Informação para a Atenção Primária à Saúde — seção “Exemplo: SISAPS + CNES”](sisaps.html#exemplo-sisaps-cnes). Na edição impressa, p. 291.
