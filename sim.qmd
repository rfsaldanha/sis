...
A versão atual da DO é organizada em nove blocos. Eles cobrem identificação, residência, ocorrência, informações específicas para óbito fetal ou menor de um ano, condições e causas do óbito, dados do médico, causas externas, cartório e localidade sem médico [@brasil_ms_2022_do_manual]. Na análise dos microdados, esses blocos aparecem como variáveis que descrevem dimensões diferentes do evento. Por isso, conhecer a estrutura do formulário ajuda a evitar erros de leitura da base.
...
Também é necessário decidir se o estudo usará óbitos totais, óbitos de residentes, óbitos ocorridos no território, óbitos não fetais ou subconjuntos específicos, como infantis, maternos ou por causas externas. Essa decisão deve aparecer no numerador do indicador e na descrição metodológica. Em geral, taxas populacionais usam óbitos de residentes; análises da rede assistencial podem usar ocorrência; e análises de mortalidade fetal ou infantil precisam separar cuidadosamente eventos antes e depois do nascimento.
...
Uma rotina mínima de avaliação de qualidade deve incluir: total de óbitos por ano e território; proporção de óbitos por causas mal definidas; completitude de idade, sexo, raça/cor, escolaridade e município; distribuição de óbitos por residência e ocorrência; proporção de óbitos investigados quando pertinente; e comparação entre bases preliminares e consolidadas. Essa etapa deve vir antes da análise substantiva, não apenas como anexo metodológico.
