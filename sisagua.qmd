...
```{=html}
<section class="podcast-player" data-transcript="podcast/sisagua_dialogo_eleven_v3_transcript.json?v=20260713-elevenv3">
...
<audio class="podcast-audio" controls preload="metadata">
  <source src="podcast/sisagua_dialogo_eleven_v3.mp3?v=20260713-elevenv3" type="audio/mpeg">
...
::: {#qua-sisagua-caracteristicas-gerais-do-sisagua}
...
::: {#qua-sisagua-perguntas-comuns-que-podem-ser-respondidas-com-o-sisagua}
...
::: {#qua-sisagua-atores-e-responsabilidades-no-fluxo-do-sisagua}
...
O @qua-sisagua-atores-e-responsabilidades-no-fluxo-do-sisagua mostra que o SISAGUA não é alimentado por um único tipo de produtor de informação. Parte dos dados expressa a responsabilidade operacional de quem abastece; outra parte expressa a ação fiscalizatória e analítica do setor saúde. Essa separação é essencial para interpretar atrasos, lacunas e divergências entre controle e vigilância.

## Módulos do sistema

Os dados do SISAGUA são organizados em três módulos principais: Cadastro, Controle e Vigilância (ver @qua-sisagua-modulos-principais-do-sisagua).

::: {#qua-sisagua-modulos-principais-do-sisagua}
...
::: {#qua-sisagua-escolha-inicial-de-bases-do-sisagua}
...
O @qua-sisagua-escolha-inicial-de-bases-do-sisagua funciona como uma triagem metodológica. Perguntas sobre cobertura começam no Cadastro; perguntas sobre cumprimento de responsabilidade operacional começam no Controle; perguntas sobre ação do setor saúde começam na Vigilância. Quando o estudo combina módulos, a etapa de integração deve preservar origem do dado, período, forma de abastecimento e parâmetro.

### Controle e Vigilância

Controle e Vigilância são complementares, mas não intercambiáveis. O Controle é executado pelo responsável pelo abastecimento; a Vigilância é executada pelo setor saúde para verificar o atendimento à norma e avaliar risco (ver @qua-sisagua-diferencas-entre-controle-e-vigilancia-no-sisagua).

::: {#qua-sisagua-diferencas-entre-controle-e-vigilancia-no-sisagua}
...
A forma de abastecimento é uma dimensão central no SISAGUA. Ela define o contexto em que a população recebe água e condiciona responsabilidades, monitoramento e interpretação de risco (ver @qua-sisagua-formas-de-abastecimento-registradas-no-sisagua).

::: {#qua-sisagua-formas-de-abastecimento-registradas-no-sisagua}
...
::: {#qua-sisagua-unidades-de-analise-frequentes-no-sisagua}
...
Esse @qua-sisagua-unidades-de-analise-frequentes-no-sisagua reforça que o SISAGUA alterna entre unidades estruturais e unidades laboratoriais. Uma forma de abastecimento pode gerar muitas amostras, uma amostra pode ter vários parâmetros e uma população abastecida pode ser estimada por cadastro. Portanto, a contagem de registros precisa ser traduzida para a unidade substantiva da pergunta antes de virar indicador.

## Estrutura dos dados

Os dados incluem cadastro das formas de abastecimento, resultados de monitoramento de controle e resultados de vigilância. Em análises reprodutíveis, registre módulo, ano, município, forma de abastecimento, ponto de coleta, parâmetro e regra de tratamento de resultados (ver @qua-sisagua-dimensoes-uteis-para-inspecao-do-sisagua).

::: {#qua-sisagua-dimensoes-uteis-para-inspecao-do-sisagua}
...
::: {#qua-sisagua-conceitos-minimos-para-analise-do-sisagua}
...
O @qua-sisagua-conceitos-minimos-para-analise-do-sisagua sintetiza os elementos discutidos. O dicionário mínimo explicita termos que costumam ser confundidos na leitura dos dados. Em especial, "fora do padrão" depende de parâmetro, unidade, ponto e norma; "ausência de amostra" é uma informação sobre monitoramento, não sobre qualidade da água. Registrar essas definições evita que o resultado seja interpretado além do que a base permite.

## Parâmetros e potabilidade

A norma de qualidade da água define parâmetros, valores de referência, planos de amostragem e responsabilidades. No SISAGUA, os parâmetros podem incluir indicadores microbiológicos, físico-químicos, desinfecção, turbidez, fluoretação, substâncias químicas e características organolépticas (ver @qua-sisagua-grupos-de-parametros-frequentes-em-analises-de-qualidade-da-agua).

::: {#qua-sisagua-grupos-de-parametros-frequentes-em-analises-de-qualidade-da-agua}
...
O número esperado de amostras não é fixo para todos os municípios, parâmetros ou formas de abastecimento. Ele depende da norma vigente, população abastecida, forma de abastecimento, parâmetro, frequência de monitoramento e ponto de coleta (ver @qua-sisagua-elementos-que-definem-o-plano-de-amostragem).

::: {#qua-sisagua-elementos-que-definem-o-plano-de-amostragem}
...
::: {.callout-note}
O exemplo assume que o plano de amostragem esperado já foi estruturado em uma tabela auxiliar. Na prática, esse quadro deve ser derivada da norma vigente e das características da forma de abastecimento.
:::
...
A qualidade do SISAGUA depende de cadastro atualizado, oportunidade do registro, consistência territorial, completitude de campos, integração com laboratórios e capacidade operacional da vigilância (ver @qua-sisagua-dimensoes-de-qualidade-relevantes-no-sisagua).

::: {#qua-sisagua-dimensoes-de-qualidade-relevantes-no-sisagua}
...
Séries temporais do SISAGUA exigem cuidado com mudanças de versão do sistema, norma de potabilidade, completitude cadastral e capacidade local de monitoramento. Comparações territoriais devem considerar diferenças nas formas de abastecimento e no porte populacional (ver @qua-sisagua-cuidados-em-analises-temporais-e-territoriais-do-sisagua).

::: {#qua-sisagua-cuidados-em-analises-temporais-e-territoriais-do-sisagua}
...
::: {#qua-sisagua-leitura-espacial-dos-pontos-no-sisagua}
...
Coletas realizadas em denúncias, surtos, enchentes, secas, desastres, intermitência de abastecimento ou uso de carro-pipa podem ser direcionadas a situações de maior risco. Elas são importantes para resposta em saúde pública, mas não devem ser comparadas diretamente com monitoramento de rotina (ver @qua-sisagua-situacoes-em-que-o-monitoramento-pode-ser-direcionado-por-risco).

::: {#qua-sisagua-situacoes-em-que-o-monitoramento-pode-ser-direcionado-por-risco}
...
O SISAGUA possui diferentes formas de disseminação: dados abertos, relatórios no sistema e painéis de informação. Dados brutos em formato CSV estão disponíveis em portais públicos, geralmente acompanhados de dicionários de dados (ver @qua-sisagua-formas-de-acesso-e-integracao-do-sisagua).

::: {#qua-sisagua-formas-de-acesso-e-integracao-do-sisagua}
...
Os dados abertos do SISAGUA podem ultrapassar limites práticos de planilhas. Para fluxos reprodutíveis, prefira ferramentas que leiam dados em partes ou consultem arquivos sem carregá-los integralmente na memória (ver @qua-sisagua-estrategias-para-processamento-de-dados-extensos-do-sisagua).

::: {#qua-sisagua-estrategias-para-processamento-de-dados-extensos-do-sisagua}
...
O SISAGUA pode ser combinado com bases de saúde, território, população e saneamento, desde que a unidade territorial e temporal esteja bem definida (ver @qua-sisagua-integracoes-frequentes-entre-sisagua-e-outros-sistemas).

::: {#qua-sisagua-integracoes-frequentes-entre-sisagua-e-outros-sistemas}
...
::: {#qua-sisagua-verificacoes-antes-de-integrar-o-sisagua-com-outras-bases}
...
O @qua-sisagua-limitacoes-recorrentes-do-sisagua sintetiza os elementos discutidos. Essas limitações delimitam o uso correto do sistema. O SISAGUA é indispensável para vigilância da qualidade da água, mas seus registros dependem de cadastro, monitoramento, laboratório e digitação. A ausência de evidência de inconformidade só deve ser interpretada junto com a evidência de que houve monitoramento suficiente e adequado.
...
::: {#qua-sisagua-erros-frequentes-em-analises-com-sisagua}
...
Os erros do @qua-sisagua-erros-frequentes-em-analises-com-sisagua têm uma origem comum: transformar dados de monitoramento em conclusões gerais sem verificar contexto. No SISAGUA, a pergunta correta raramente é apenas "quantas amostras deram fora do padrão"; é também "onde, quando, por quem, com qual parâmetro, em qual ponto, sob qual norma e com que intensidade de amostragem".
...
::: {#qua-sisagua-itens-minimos-para-reprodutibilidade-em-analises-do-sisagua}
...
A reprodutibilidade no SISAGUA depende de registrar escolhas normativas e operacionais, não apenas o código usado. Dois estudos podem baixar a mesma base e chegar a resultados diferentes se escolherem módulos, pontos de coleta, formas de abastecimento ou regras para ausentes de maneira distinta. O @qua-sisagua-itens-minimos-para-reprodutibilidade-em-analises-do-sisagua oferece um roteiro mínimo para tornar essas escolhas auditáveis.
...
## Veja também {#sec-veja-tambem-sisagua}

- [Organização dos SIS — seção “Níveis geográficos”](organizacao.html#níveis-geográficos). Na edição impressa, p. 35.

- [CNES — Cadastro Nacional de Estabelecimentos de Saúde — seção “Geocodificação e análise espacial”](cnes.html#geocodificação-e-análise-espacial). Na edição impressa, p. 213.
