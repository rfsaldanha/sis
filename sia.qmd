...
```{=html}
<section class="podcast-player" data-transcript="podcast/sia_transcript.json?v=20260712-elevenv3">
...
<audio class="podcast-audio" controls preload="metadata">
  <source src="https://sis-podcasts.nyc3.cdn.digitaloceanspaces.com/sia.mp3?v=20260712-elevenv3" type="audio/mpeg">
...
::: {#qua-sia-caracteristicas-gerais-do-sia-sus}
...
::: {#qua-sia-perguntas-comuns-que-podem-ser-respondidas-com-o-sia}
...
O @qua-sia-perguntas-comuns-que-podem-ser-respondidas-com-o-sia delimita o uso mais seguro do SIA. Em geral, ele responde bem a perguntas sobre volume, distribuição e remuneração aprovada da produção ambulatorial. Quando a pergunta envolve capacidade instalada, população exposta, custo real, diagnóstico completo ou desfecho, o SIA raramente basta sozinho: ele precisa ser articulado a outras fontes e a uma definição explícita da unidade de análise.
...
Em 2016, a Comissão Intergestores Tripartite instituiu o Conjunto Mínimo de Dados da Atenção à Saúde (CMD), com implantação incremental e gradual e previsão de integração dos registros do SIA, do SIH e de sistemas relacionados [@brasilResolucaoCMD2016]. Em 2025, a Portaria GM/MS nº 7.639 incluiu o SIA e o CMD entre os bancos nacionais cuja alimentação deve ocorrer de forma mensal e sistemática [@brasilPortaria76392025]. Esses marcos registram uma agenda de convergência informacional, não a descontinuação imediata do SIA.

## SIA e SIH

SIA e SIH (@sec-sih) são sistemas de produção assistencial e remuneração, mas registram tipos diferentes de cuidado (ver @qua-sia-diferencas-praticas-entre-sia-e-sih).

::: {#qua-sia-diferencas-praticas-entre-sia-e-sih}
...
%%| label: fig-sia-fluxo
%%| fig-alt: "Fluxograma do SIA: atendimentos e procedimentos são registrados em BPA, APAC ou RAAS, enviados ao gestor e submetidos a críticas; a produção aprovada integra a disseminação do sistema e a rejeitada não a integra."
flowchart TD
...
O SIA reúne instrumentos com diferentes níveis de detalhamento. A escolha do arquivo deve seguir a pergunta da análise (ver @qua-sia-instrumentos-de-registro-do-sia).

::: {#qua-sia-instrumentos-de-registro-do-sia}
...
::: {#qua-sia-unidades-de-analise-possiveis-no-sia}
...
O código do procedimento é um dos campos centrais do SIA. Ele é gerenciado pela tabela SIGTAP, que define descrição, grupo, subgrupo, forma de organização, complexidade, financiamento, valores e regras de compatibilidade (ver @qua-sia-elementos-do-sigtap-relevantes-para-analise-do-sia).

::: {#qua-sia-elementos-do-sigtap-relevantes-para-analise-do-sia}
...
O SIA permite analisar produção por estabelecimento, município de ocorrência, município de residência, competência e, em alguns instrumentos, informações individualizadas do usuário. Cada recorte responde a uma pergunta diferente (ver @qua-sia-recortes-territoriais-e-temporais-frequentes-no-sia).

::: {#qua-sia-recortes-territoriais-e-temporais-frequentes-no-sia}
...
Mamografias são um exemplo de uso do SIA para análise de produção ambulatorial. A pergunta pode envolver volume de exames, distribuição territorial, uso por faixa etária, concentração em estabelecimentos e relação entre residência e ocorrência (ver @qua-sia-codigos-sigtap-comuns-em-analises-de-mamografia).

::: {#qua-sia-codigos-sigtap-comuns-em-analises-de-mamografia}
...
A distinção entre os dois códigos é substantiva. O primeiro pode reunir mamografias com finalidades clínicas variadas; o segundo é mais alinhado ao rastreamento. Se a pergunta for cobertura de rastreamento, misturar os códigos pode inflar o numerador ou mudar seu significado. Se a pergunta for capacidade de oferta de exames, incluir ambos pode ser adequado, desde que a finalidade seja declarada.

::: {#qua-sia-campos-uteis-em-uma-analise-de-mamografias-no-sia}
...
Um indicador aplicado pode estimar a taxa de mamografias de rastreamento em mulheres de 50 a 69 anos residentes em determinado município. O numerador viria do SIA por residência; o denominador, de estimativas populacionais femininas na mesma faixa etária (ver @qua-sia-exemplo-de-especificacao-de-indicador-com-sia).

::: {#qua-sia-exemplo-de-especificacao-de-indicador-com-sia}
...
Os nomes e a disponibilidade das variáveis variam conforme PA, APAC, BPA-I e RAAS. O @qua-sia-campos-uteis-para-uma-primeira-inspecao-dos-dados-do-sia reúne os campos que costumam orientar uma primeira inspeção.

::: {#qua-sia-campos-uteis-para-uma-primeira-inspecao-dos-dados-do-sia}
...
Muitas análises do SIA dependem de transformar campos originais em categorias ou medidas. Essas regras devem ser documentadas no método (ver @qua-sia-transformacoes-comuns-em-analises-com-sia).

::: {#qua-sia-transformacoes-comuns-em-analises-com-sia}
...
Os dados do SIA são distribuídos em várias famílias de arquivos. A lista abaixo resume prefixos comuns (ver @qua-sia-prefixos-comuns-dos-arquivos-do-sia).

::: {#qua-sia-prefixos-comuns-dos-arquivos-do-sia}
...
Ao trabalhar com microdados do SIA, não assuma que todos os arquivos possuem os mesmos campos. Uma estratégia prática é organizar o dicionário por blocos (ver @qua-sia-blocos-uteis-para-organizar-um-dicionario-de-dados-do-sia).

::: {#qua-sia-blocos-uteis-para-organizar-um-dicionario-de-dados-do-sia}
...
A APAC registra procedimentos ambulatoriais de alta complexidade ou alto custo, como medicamentos especializados, terapias renais, quimioterapia, radioterapia e outros tratamentos específicos. Em geral, a APAC exige autorização, laudo e regras próprias de continuidade (ver @qua-sia-cuidados-em-analises-com-apac).

::: {#qua-sia-cuidados-em-analises-com-apac}
...
Exemplos frequentes de análise com APAC incluem tratamentos oncológicos, terapia renal substitutiva, radioterapia, quimioterapia, medicamentos especializados e procedimentos com autorização continuada. Nesses casos, a competência mensal pode representar continuidade de cuidado, não um novo caso incidente (ver @qua-sia-exemplos-de-uso-analitico-da-apac).

::: {#qua-sia-exemplos-de-uso-analitico-da-apac}
...
Assim como nos demais sistemas de produção, registros podem ser rejeitados por inconsistências, incompatibilidades ou problemas administrativos. Quando o objetivo é medir produção realizada e aprovada, use arquivos de produção aprovada. Quando o objetivo é auditoria ou qualidade do faturamento, analise rejeições separadamente (ver @qua-sia-uso-analitico-de-aprovacao-e-rejeicao-no-sia).
...
Os dados do SIA podem ser acessados por diferentes caminhos, conforme o objetivo da análise (ver @qua-sia-formas-de-acesso-aos-dados-do-sia).

::: {#qua-sia-formas-de-acesso-aos-dados-do-sia}
...
A melhor forma de acesso depende do grau de reprodutibilidade necessário. O TabNet é adequado para o reconhecimento inicial e validação de totais. Arquivos DBC, R, Python e PCDaS são mais adequados quando a análise precisa documentar filtros, combinar várias competências, repetir o processamento ou integrar o SIA a outras bases.

## Processamento de grandes volumes

O SIA é disseminado em arquivos mensais por unidade federativa e por instrumento. Séries longas e análises nacionais podem envolver grande volume e diferentes estruturas de arquivos (ver @qua-sia-estrategias-para-processamento-do-sia-em-escala).

::: {#qua-sia-estrategias-para-processamento-do-sia-em-escala}
...
O SIA é frequentemente combinado com outros sistemas para caracterizar estabelecimentos, rede, denominadores e desfechos (ver @qua-sia-integracoes-frequentes-entre-o-sia-e-outros-sistemas).

::: {#qua-sia-integracoes-frequentes-entre-o-sia-e-outros-sistemas}
...
::: {#qua-sia-checagens-automaticas-uteis-antes-da-analise}
...
As checagens automáticas reunidas no @qua-sia-checagens-automaticas-uteis-antes-da-analise devem ser complementadas pela comparação dos totais agregados com o TabNet no mesmo período, UF, instrumento e procedimento. Diferenças pequenas podem resultar de filtros, competência ou processamento; diferenças grandes geralmente indicam seleção incorreta de arquivos ou variáveis.

## Principais usos e indicadores

Os dados do SIA são usados em indicadores de produção ambulatorial, acesso a procedimentos, oferta regional, uso de serviços especializados e valores aprovados (ver @qua-sia-exemplos-de-indicadores-construidos-com-dados-do-sia).

::: {#qua-sia-exemplos-de-indicadores-construidos-com-dados-do-sia}
...
O SIA é uma fonte ampla e valiosa, mas suas limitações precisam aparecer explicitamente no método e na discussão (ver @qua-sia-limitacoes-recorrentes-do-sia).

::: {#qua-sia-limitacoes-recorrentes-do-sia}
...
::: {#qua-sia-checklist-de-qualidade-para-analises-com-sia}
...
O *checklist* do @qua-sia-checklist-de-qualidade-para-analises-com-sia serve como revisão final da coerência entre pergunta, fonte e indicador. Ele não substitui a leitura do dicionário nem a validação contra totais agregados, mas ajuda a identificar rapidamente se a análise trocou unidade, território ou sentido do valor registrado.
...
::: {#qua-sia-erros-frequentes-em-analises-do-sia}
...
Os erros reunidos no @qua-sia-erros-frequentes-em-analises-do-sia são simples de formular, mas têm grande impacto nos resultados. A forma mais eficiente de preveni-los é escrever, antes do processamento, uma especificação curta com arquivo usado, filtros, unidade de análise, campos somados, território e comparação de validação esperada.
...
## Veja também {#sec-veja-tambem-sia}

- [Eventos de saúde — seção “Procedimentos ambulatoriais”](eventos.html#procedimentos-ambulatoriais). Na edição impressa, p. 58.

- [SIH — Sistema de Informações Hospitalares do SUS — seção “SIH e SIA”](sih.html#sih-e-sia). Na edição impressa, p. 115.
...
- [SISCAN — Sistema de Informação do Câncer — seção “Integração com SIA/SUS, CNES e CadSUS”](siscan.html#integração-com-siasus-cnes-e-cadsus). Na edição impressa, p. 224.
