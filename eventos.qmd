...
::: {.callout}
Um exemplo: uma pessoa doente pode acessar o sistema de saúde em uma Unidade de Pronto Atendimento (UPA), receber cuidados iniciais e ser diagnosticado como um caso de dengue. Em seguida, esse paciente pode apresentar uma piora em seu quadro clínico e ser internado no hospital, evoluindo para óbito. Neste exemplo, temos quatro eventos de saúde: (1) o atendimento ambulatorial para tratamento sintomático e diagnóstico, (2) a notificação da ocorrência de um caso grave de dengue, (3) a internação hospitalar e (4) o óbito. 
:::

Observe que cada um desses eventos é registrado em diferentes SIS, como será visto a seguir. A seguir, são apresentadas características dos principais eventos de saúde e seus respectivos SIS.
...
Doenças e agravos podem levar ao óbito, que é um importante evento de saúde. A ocorrência de óbitos evitáveis podem sinalizam falhas no sistema de saúde que devem ser corrigidas. Os eventos de óbito são registrados no Sistema de Informações de Mortalidade -- SIM (@sec-sim).

Uma importante definição neste evento é a **causa básica do óbito**, que é o agravo e circunstância que inicia a cadeia de eventos que leva a morte do paciente. As várias circunstâncias e comorbidades que contribuem para o desfecho de óbito são registrados, com especial destaque para a sua causa básica.

## Nascimentos

O nascimento de uma criança é visto como um evento de saúde que abrange tanto a mãe como o próprio recém-nascido. Os nascimentos são registrados no Sistema de Informações de Nascidos Vivos -- SINASC (@sec-sinasc), que segue uma definição específica sobre "nascidos vivos":
...
Os eventos de óbitos fetais são registrados no SIM (@sec-sim).

## Imunizações

A imunização por vacinação é uma importante medida de profilaxia, constituindo um relevante evento de saúde para prevenção de doenças. Dados de vacinação são registrados no Sistema de Informações do Programa Nacional de Imunizações -- SI-PNI (@sec-sipni).
