...
O quadro a seguir resume esse ciclo. Ela ajuda a localizar onde podem surgir diferenças entre o momento em que algo aconteceu e o momento em que esse algo aparece na base disponível para análise. Em muitos estudos, essa diferença é decisiva para interpretar tendências recentes, comparar fontes e distinguir variação real de variação produzida pelo fluxo de informação (ver @qua-eventos-etapas-entre-a-ocorrencia-de-um-evento-e-sua-disseminacao-como-dado).
...
Na figura, a mesma trajetória clínica se desdobra em registros diferentes. O atendimento ambulatorial pode existir mesmo sem confirmação laboratorial; a notificação pode ocorrer antes da investigação completa; a internação tende a representar uma fração mais grave dos casos; e o óbito depende de regras de declaração, codificação e seleção de causa. O quadro a seguir traduz essa trajetória em sistemas e usos analíticos.
...
O quadro abaixo destaca algumas entradas possíveis para estudar acidentes de trânsito. O objetivo não é esgotar todas as fontes, mas mostrar que a escolha do sistema depende da dimensão analisada: cuidado prestado, gravidade, desfecho, local de atendimento ou estrutura da rede (ver @qua-eventos-eventos-relacionados-a-acidentes-de-transito-e-seus-sistemas-de-regist).
...
O quadro seguinte apresenta perguntas que dependem dessa integração. Ele evidencia que o relacionamento de bases não é um fim em si mesmo: ele é necessário quando o evento de interesse começa em um sistema e seu desfecho, contexto ou denominador está em outro (ver @qua-eventos-exemplos-de-perguntas-que-podem-exigir-relacionamento-entre-sis).
...
- [SINAN — Sistema de Informação de Agravos de Notificação — seção “Definições de caso”](sinan.html#definições-de-caso). Na edição impressa, p. 153.
