...
# SINAN — Sistema de Informações de Agravos de Notificação {#sec-sinan}
...
<audio class="podcast-audio" controls preload="metadata">
  <source src="https://sis-podcasts.nyc3.cdn.digitaloceanspaces.com/sinan_dialogo_eleven_v3.mp3?v=20260712-elevenv3" type="audio/mpeg">
...
O Sistema de Informações de Agravos de Notificação (SINAN) é uma das principais fontes da vigilância epidemiológica no Brasil. Ele registra doenças, agravos e eventos de saúde pública que exigem notificação, investigação, monitoramento ou resposta do sistema de saúde. A forma singular, “Sistema de Informação de Agravos de Notificação”, também aparece em normas e páginas institucionais; esta edição adota no texto corrente o título da página central do sistema mantida pelo Ministério da Saúde.
...
%%| fig-alt: "Fluxograma do SINAN: um caso suspeito ou confirmado é notificado pela unidade e percorre as secretarias municipal e estadual até o Ministério da Saúde; a investigação leva à classificação final e ao encerramento ou acompanhamento."
...
Em 2025, as versões 2.x ampliaram as funções do e-SUS SINAN. A versão 2.0.2 incorporou a ficha de notificação/conclusão para doenças e agravos sem ficha específica de investigação, e o histórico oficial da plataforma registra ainda a versão 2.3.0 no mesmo ano [@secretariadesaudeRJESUSSinan2025; @ministeriodasaudeESUSSinan2026].
...
O SINAN apresenta dados para *todos os casos suspeitos notificados*, independentemente do diagnóstico final. Assim, para obter o total de casos *confirmados* para determinada doença ou agravo, deve-se filtrar os dados segundo o diagnóstico final.
...
::: {.callout-note}
Uma possível utilidade para se manter o registro dos casos descartados é o cálculo do *Indicador de Positividade*, que avalia a proporção de casos confirmados entre os casos notificados.

$$
pos = \frac{c_c}{c_c + c_d + c_i}
$$

Nessa expressão, $c_c$ representa os casos confirmados; $c_d$, os casos descartados; e $c_i$, os casos com diagnóstico inconclusivo.
:::

A confirmação de casos, em geral, pode ser feita por exames laboratoriais, testes rápidos, critérios clínicos, critérios epidemiológicos ou combinação desses elementos, conforme a definição de caso vigente. A confirmação clínico-epidemiológica costuma ser mais adotada em situações de surtos e epidemias, quando há limitação de exames disponíveis. Em casos suspeitos de dengue, por exemplo, um caso pode ser confirmado a partir de sintomas e vínculo epidemiológico com um caso confirmado laboratorialmente.
...
Casos em que o município ou a UF de provável infecção coincide com o município ou a UF de residência podem ser classificados como autóctones, isto é, a infecção provavelmente ocorreu no território de residência. Quando o local provável de infecção é diferente da residência, podem ser classificados como alóctones.
...
![Fluxo de notificação de violências no Sistema de Informações de Agravos de Notificação (Sinan) – Componente Contínuo da Vigilância de Violências e Acidentes (Viva Sinan)](assets/sinan/img/fluxo_violencia.png){#fluxo_violencia}
...
As formas de acesso diferem em atualização, granularidade, facilidade de uso e rastreabilidade. TabNet é adequado para consulta rápida, mas limita a reprodutibilidade e controle fino dos filtros. Microdados em R, Python, OpenDataSUS ou DBC permitem maior transparência metodológica, desde que o analista documente data de acesso, leiaute, filtros, versão do código e transformação aplicada.
...
Em análises reprodutíveis, registre o agravo, prefixo do arquivo, UF, ano, data de acesso e versão do código de leitura. Como os leiautes variam por agravo, guarde também o dicionário de dados usado.
...
- [SIVEP — família de sistemas de vigilância epidemiológica — seção “SIVEP-Gripe”](sivep.html#sivep-gripe). Na edição impressa, p. 425.

- [Interoperabilidade entre os Sistemas de Informação em Saúde — seção “Etapas de um *linkage*”](interoperabilidade.html#etapas-de-um-linkage). Na edição impressa, p. 452.

- [Inquéritos e pesquisas de saúde — seção “VIVA: sistema e inquérito”](inqueritos.html#viva-sistema-e-inquérito). Na edição impressa, p. 415.
