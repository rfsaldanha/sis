...
O @qua-sinan-escolha-de-datas-para-analises-com-sinan sintetiza os elementos discutidos. Para acompanhamento epidemiológico, é comum usar data de início de sintomas, semana epidemiológica ou data provável de infecção, a depender do agravo. Para taxas populacionais, use município de residência; para investigação de transmissão, use local provável de infecção; para gestão de serviços, use município ou unidade notificadora.
...
Dada a diversidade de doenças e agravos cobertos pelo SINAN, a consulta fica mais fácil quando os arquivos são agrupados por área de vigilância. O quadro abaixo é um guia rápido; os documentos detalhados aparecem na sequência (ver @qua-sinan-guia-rapido-de-agravos-e-prefixos-do-sinan).
...
O @qua-sinan-limitacoes-do-sinan-por-tipo-de-uso-analitico sintetiza os elementos discutidos. As limitações mudam de peso conforme o uso. Para monitoramento oportuno, o atraso recente pode ser o principal problema; para séries históricas, mudanças de definição e sensibilidade da vigilância ganham importância; para letalidade, o desfecho pode exigir vínculo com o SIM. Por isso, a seção de métodos deve vincular cada limitação ao indicador produzido, em vez de listar ressalvas genéricas.
...
O @qua-sinan-erros-frequentes-em-analises-do-sinan sintetiza os elementos discutidos. Esses erros têm em comum a perda do processo de vigilância que produziu o registro. A análise fica mais robusta quando o texto diferencia suspeição, investigação, confirmação, encerramento e atualização da base. Em produtos públicos, essa distinção deve aparecer nas legendas, notas metodológicas e interpretação dos resultados, especialmente quando há comparação temporal ou territorial.
...
- [Eventos de saúde — seção “Casos de doenças e agravos”](eventos.html#casos-de-doenças-e-agravos). Na edição impressa, p. 48.

- [SIVEP — Sistema de Vigilância Epidemiológica — seção “SIVEP-Gripe”](sivep.html#sivep-gripe). Na edição impressa, p. 395.

- [Interoperabilidade entre os Sistemas de Informação em Saúde — seção “Etapas de um *linkage*”](interoperabilidade.html#etapas-de-um-linkage). Na edição impressa, p. 422.
