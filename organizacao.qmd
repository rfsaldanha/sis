...
<audio class="podcast-audio" controls preload="metadata">
  <source src="https://sis-podcasts.nyc3.cdn.digitaloceanspaces.com/organizacao_dialogo_eleven_v3.mp3" type="audio/mpeg">
...
Os sistemas vitais registram eventos diretamente relacionados ao início e ao fim da vida. No Brasil, os principais são o Sistema de Informações sobre Mortalidade (SIM) (@sec-sim) e o Sistema de Informações sobre Nascidos Vivos (SINASC) (@sec-sinasc).
...
Também entram nesse conjunto sistemas e subsistemas de vigilância específicos, como os componentes agrupados sob a sigla SIVEP (@sec-sivep), voltados a agravos ou síndromes específicos, e o Sistema de Informação de Vigilância da Qualidade da Água para Consumo Humano (SISAGUA) (@sec-sisagua).
...
O ponto comum entre esses sistemas é a função de vigilância, mas cada linha do @qua-organizacao-sistemas-de-morbidade-e-vigilancia-abordados-no-livro representa uma lógica de registro diferente. No SINAN, a ficha pode iniciar com uma suspeita e só depois receber classificação final. No SIVEP, a definição de caso e o conjunto de variáveis dependem do componente analisado. No SISAGUA, o foco não é uma pessoa doente, mas uma condição ambiental de interesse para a saúde coletiva.
...
O Sistema de Informação para a Atenção Primária à Saúde (Siaps) (@sec-siaps), em transição a partir do SISAB, organiza informações da APS, incluindo equipes, vínculo territorial, produção e indicadores relacionados à estratégia e-SUS APS.
...
| SISAB e Siaps (@sec-siaps) | Produção, vínculo territorial e indicadores da APS | Gestão, monitoramento, cofinanciamento e avaliação da APS |
...
Outros sistemas registram principalmente eventos, produção ou gestão vinculados à dimensão pública do SUS, incluindo rede própria e rede conveniada. SIH (@sec-sih), SIA (@sec-sia), SISCAN (@sec-siscan), SIOPS (@sec-siops), SISAB e Siaps (@sec-siaps) e SI-PNI (@sec-sipni) devem ser interpretados com essa delimitação em mente.
...
Indicadores calculados a partir dos SIS também carregam essas escolhas. A construção de um indicador exige definir numerador, denominador, população de referência, período, território, fonte de dados e critérios de inclusão e exclusão. Essa ficha conceitual é tão importante quanto o cálculo em si, porque permite que outras pessoas compreendam e reproduzam a análise [@OPAS2026; @sobralDefinicoesBasicasDado2011].
...
- [Eventos de saúde — seção “Relação entre eventos e perguntas de análise”](eventos.html#relação-entre-eventos-e-perguntas-de-análise). Na edição impressa, p. 63.

- [Breve histórico da experiência brasileira — seção “Marcos de criação dos principais sistemas”](historico.html#marcos-de-criação-dos-principais-sistemas). Na edição impressa, p. 24.
