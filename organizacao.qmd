...
Outros sistemas registram principalmente eventos, produção ou gestão vinculados à dimensão pública do SUS, incluindo rede própria e rede conveniada. SIH (@sec-sih), SIA (@sec-sia), SIOPS (@sec-siops), SISAB/SISAPS (@sec-siaps) e SI-PNI (@sec-sipni) devem ser interpretados com essa delimitação em mente.
...
- [Interoperabilidade entre os Sistemas de Informação em Saúde — seção “Aplicação aos SIS”](interoperabilidade.html#aplicação-aos-sis). Na edição impressa, p. 426.
