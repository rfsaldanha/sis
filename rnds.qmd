...
<audio class="podcast-audio" controls preload="metadata">
  <source src="https://sis-podcasts.nyc3.cdn.digitaloceanspaces.com/rnds.mp3" type="audio/mpeg">
...
Essa característica torna a RNDS diferente das bases tradicionalmente usadas em epidemiologia e gestão. Quando se trabalha com SIM ou SINASC, por exemplo, a pergunta inicial costuma ser "qual evento foi registrado?". Na RNDS, a pergunta inicial precisa ser anterior: "qual fluxo de informação gerou este registro e por qual modelo ele foi compartilhado?". A resposta condiciona a interpretação da cobertura, do atraso, da duplicidade, da completitude e da finalidade de uso.
...
%%| fig-alt: "Fluxograma da RNDS: o dado do serviço passa pelo sistema de origem, pelo modelo de informação e recurso FHIR, por autenticação, envio e validações; então integra a RNDS e seus usos por cidadãos, profissionais e gestores."
flowchart TD
...
Terminologias controladas, como LOINC para exames laboratoriais, ajudam a reduzir a ambiguidade e a permitir uma interpretação comum entre sistemas. Ainda assim, a existência de um código não garante que o registro local tenha sido produzido corretamente.
...
| SISAB e Siaps (@sec-siaps) | Registros da APS podem compor fluxos de atendimento e vacinação | Fluxos intermediários podem gerar atraso |
...
- [Interoperabilidade entre os Sistemas de Informação em Saúde — seção “O que é interoperabilidade”](interoperabilidade.html#o-que-é-interoperabilidade). Na edição impressa, p. 447.

- [Organização dos SIS — seção “Integração entre sistemas”](organizacao.html#integração-entre-sistemas). Na edição impressa, p. 47.

- [SI-PNI — Sistema de Informações do Programa Nacional de Imunizações — seção “Privacidade e dados individualizados”](sipni.html#privacidade-e-dados-individualizados). Na edição impressa, p. 330.
