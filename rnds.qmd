...
O @qua-rnds-escopo-conceitual-da-rnds sintetiza os elementos discutidos. Essa distinção é importante. A RNDS conecta sistemas, mas a qualidade do que circula depende do registro original, do modelo de informação, da identificação correta da pessoa, da conformidade técnica, da governança e do uso adequado por profissionais e gestores.
...
O @qua-rnds-camadas-conceituais-da-rnds de camadas ajuda a localizar problemas. Se um registro de vacinação não aparece, o problema pode estar na sala de vacina, no sistema de registro, no envio à RNDS, na validação, na identificação do cidadão ou na interface de consulta. Cada camada exige uma investigação diferente. Cobrar apenas a "RNDS" como se fosse uma única base estática pode esconder a etapa real em que ocorreu a falha.
...
O @qua-rnds-perguntas-que-exigem-rnds-combinada-com-outras-fontes sintetiza os elementos discutidos. A RNDS tende a ser mais forte quando a pergunta envolve conexão entre registros. Ela tende a ser insuficiente quando a pergunta exige denominadores populacionais, definição epidemiológica de caso, avaliação clínica aprofundada, mensuração de toda a produção assistencial ou análise econômica completa. Nesses casos, a plataforma pode contribuir, mas precisa ser combinada com sistemas finalísticos, cadastros, inquéritos, auditorias, protocolos ou dados locais.
...
- [Interoperabilidade entre os Sistemas de Informação em Saúde — seção “O que é interoperabilidade”](interoperabilidade.html#o-que-é-interoperabilidade). Na edição impressa, p. 411.

- [Organização dos SIS — seção “Integração entre sistemas”](organizacao.html#integração-entre-sistemas). Na edição impressa, p. 38.

- [SI-PNI — Sistema de Informações do Programa Nacional de Imunizações — seção “Privacidade e dados individualizados”](sipni.html#privacidade-e-dados-individualizados). Na edição impressa, p. 320.
