...
<audio class="podcast-audio" controls preload="metadata">
  <source src="https://sis-podcasts.nyc3.cdn.digitaloceanspaces.com/sipni.mp3?v=20260713-elevenv3" type="audio/mpeg">
...
O programa é o PNI; o sistema de informação é usualmente chamado SI-PNI ou SIPNI. Documentos institucionais alternam as formas “Sistema de Informação” e “Sistema de Informações”; esta edição adota “Sistema de Informações do Programa Nacional de Imunizações”, denominação exibida no portal do SI-PNI. Neste capítulo, SI-PNI é usado como nome do sistema, preservando a forma mais comum nos capítulos do livro.
:::

## Como ler este capítulo

O @qua-sipni-formas-de-uso-do-capitulo-por-diferentes-publicos organiza os quatro usos complementares para os quais este capítulo foi escrito.
...
Em 31 de maio de 2023, foi encerrada a inserção de dados nos módulos SI-PNI web e desktop para o Registro de Vacinação Individualizado e Movimentação de Imunobiológicos. A partir de 1º de junho de 2023, o Ministério da Saúde disponibilizou o módulo de vacinação de rotina no novo SI-PNI, direcionando o registro para a nova plataforma integrada à RNDS [@brasilNovoSIPNI2023].

Em 2024, a Portaria GM/MS nº 5.663 determinou que os sistemas de registro de vacinação encaminhassem os dados de doses aplicadas exclusivamente à RNDS, conforme o modelo vigente de Registro de Imunobiológico Aplicado (RIA). A norma manteve a possibilidade de uso do SI-PNI, do e-SUS APS e de sistemas próprios ou de terceiros, desde que observadas as regras de interoperabilidade com a RNDS [@brasilPortaria56632024].

::: {.content-visible when-format="html"}
```{r}
#| echo: false
#| message: false
#| fig-cap: "Linha do tempo do SI-PNI"
#| column: page
timevis::timevis(readr::read_csv2(file = "linha_tempo_sipni.csv"))
```
:::
...
%%| fig-alt: "Fluxograma do SI-PNI: a vacinação é registrada no novo SI-PNI, em sistema local integrado ou no e-SUS APS; os registros convergem para a RNDS e alimentam bases e relatórios de gestão, vigilância, cobertura e pesquisa."
flowchart TD
...
Essa distinção ajuda a evitar conclusões que extrapolem os dados. Uma queda de doses registradas pode indicar redução real da vacinação, mas também pode indicar atraso de envio, mudança de sistema, problema de integração, erro de filtro, alteração de calendário ou mudança na população-alvo.
...
Os indicadores mais comuns usam conceitos diferentes, que o @qua-sipni-diferencas-entre-doses-pessoas-esquemas-e-cobertura distingue segundo numerador, denominador e cuidados de interpretação.
...
O resultado pode gerar três medidas diferentes: total de registros de dose, pessoas com pelo menos uma dose e pessoas com esquema completo. As três podem ser corretas, desde que o texto as denomine com precisão.
...
| A APS está registrando vacinação? | SI-PNI + (SISAB ou Siaps) (@sec-siaps) | Sistemas podem ter fluxos e prazos diferentes |
...
- Combinar com SINASC, POP, CNES, SISAB, Siaps ou SINAN apenas quando a unidade de análise estiver clara.
...
- [Eventos de saúde — seção “Imunizações”](eventos.html#imunizações). Na edição impressa, p. 61.

- [Estimativas populacionais — seção “Escolha do denominador”](pop.html#escolha-do-denominador). Na edição impressa, p. 403.

- [RNDS — Rede Nacional de Dados em Saúde — seção “Modelos de informação”](rnds.html#modelos-de-informação). Na edição impressa, p. 345.
